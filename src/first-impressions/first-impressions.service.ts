...
export class FirstImpressionsService {
  ...
  async send(senderId: string, receiverId: string, rawContent: string) {
    ...
    if (relation?.status === MatchStatus.BLOCKED) {
      throw new ForbiddenException('You cannot send a First Impression to a blocked profile.');
    }

    const sender = await this.users.findOne({ where: { id: senderId }, select: ['id', 'gender'] });
    const womenDailyLimit = sender && isWoman(sender) ? 10 : undefined;
    const quota = await this.planUsage.assertAndRecord(
      senderId,
      'firstImpressionsPerMonth',
      'First Impression',
      receiverId,
      Boolean(womenDailyLimit),
      womenDailyLimit,
    );
    ...
    } else if (relation.status === MatchStatus.DECLINED) {
      relation.senderId = senderId;
      relation.receiverId = receiverId;
      relation.status = MatchStatus.PENDING;
