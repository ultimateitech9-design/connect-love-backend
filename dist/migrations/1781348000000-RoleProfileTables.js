...
Object.defineProperty(exports, "RoleProfileTables1781348000000", {
    enumerable: true,
    get: function() {
        return RoleProfileTables1781348000000;
    }
});
const ROLE_TABLES = [
    [
...
let RoleProfileTables1781348000000 = class RoleProfileTables1781348000000 {
    async up(queryRunner) {
        ...
        const insertStatements = ROLE_TABLES.map(([role, table], index)=>`${index === 0 ? 'IF' : 'ELSEIF'} NEW.role = '${role}' THEN INSERT IGNORE INTO ${table} (userId) VALUES (NEW.id);`).join(' ');
      ...
      BEGIN ${insertStatements} END IF; END`);
        ...
        const updateStatements = ROLE_TABLES.map(([role, table], index)=>`${index === 0 ? 'IF' : 'ELSEIF'} NEW.role = '${role}' THEN INSERT IGNORE INTO ${table} (userId) VALUES (NEW.id);`).join(' ');
    ...
    constructor(){
        this.name = 'RoleProfileTables1781348000000';
    }
};

//# sourceMappingURL=1781348000000-RoleProfileTables.js.map